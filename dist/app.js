...
async function init(){try{const data=await api('/api/auth/me');$('#googleLogin').classList.toggle('hidden',!data.googleEnabled);if(!data.user){localStorage.removeItem('qubi-user');return showAuth()}user=data.user;await showApp()}catch{const cached=localStorage.getItem('qubi-user');if(cached&&!navigator.onLine){user=JSON.parse(cached);await showApp()}else showAuth()}const params=new URLSearchParams(location.search);if(params.get('auth')){$('#authError').textContent=trMsg(params.get('auth')==='failed'?d('googleAuthFailed'):d('googleAuthNotConfigured'));history.replaceState({},'',location.pathname)}}
