...
const staticText={
 en:{
  ...
  'KÖZÖSSÉG':'COMMUNITY','Barátok & Chat':'Friends & Chat','💌 Barát meghívása':'💌 Invite a friend','Beérkező barátkérelmek':'Incoming friend requests','Barátaim':'My Friends','Válassz barátot a beszélgetéshez!':'Select a friend to start chatting!','Írj üzenetet...':'Write a message...','Küldés':'Send','KAPCSOLATÉPÍTÉS':'NETWORKING','Barát meghívása':'Invite a friend','Add meg a barátod email-címét. Ha már regisztrált a Qubi-ban, barátkérelmet kap. Ha még nem, meghívó e-mailt küldünk neki!':'Enter your friend\'s email address. If they already use Qubi, they will receive a friend request. If not, we will send them an invitation email!','Barátod email-címe':'Friend\'s email address','💌 Meghívó küldése':'💌 Send invitation','Elfogadás':'Accept','Elutasítás':'Decline','Még nincs elfogadott barátod. Hívd meg őket a fenti gombbal!':'You have no accepted friends yet. Invite them with the button above!',
 ...
 uk:{
  ...
  'KÖZÖSSÉG':'СПІЛЬНОТА','Barátok & Chat':'Друзі та Чат','💌 Barát meghívása':'💌 Запросити друга','Beérkező barátkérelmek':'Вхідні запити в друзі','Barátaim':'Мої друзі','Válassz barátot a beszélgetéshez!':'Виберіть друга, щоб розпочати чат!','Írj üzenetet...':'Напишіть повідомлення...','Küldés':'Надіслати','KAPCSOLATÉPÍTÉS':'СВ’ЯЗОК','Barát meghívása':'Запросити друга','Add meg a barátod email-címét. Ha már regisztrált a Qubi-ban, barátkérelmet kap. Ha még nem, meghívó e-mailt küldünk neki!':'Вкажіть email вашого друга. Якщо він вже має акаунт в Qubi, він отримає запит. Якщо ні, ми надішлемо йому запрошення!','Barátod email-címe':'Email друга','💌 Meghívó küldése':'💌 Надіслати запрошення','Elfogadás':'Прийняти','Elutasítás':'Відхилити','Még nincs elfogadott barátod. Hívd meg őket a fenti gombbal!':'У вас ще немає підтверджених друзів. Запросіть їх кнопкою вище!',
...
async function loadFriends() {
 ...
 try {
   ...
   reqList.replaceChildren(...pendingRequests.map(r => {
    ...
    item.innerHTML = `<div><strong>${escapeHtml(r.display_name)}</strong><small style="display:block;color:var(--muted);">${escapeHtml(r.email)}</small></div>
      <div style="display:flex;gap:6px;">
        <button class="primary-btn accept-btn" style="width:auto;padding:6px 14px;font-size:0.78rem;">${trMsg('Elfogadás')}</button>
        <button class="secondary-btn reject-btn" style="width:auto;padding:6px 14px;font-size:0.78rem;">${trMsg('Elutasítás')}</button>
      </div>`;
  ...
  if (acceptedFriends.length === 0) {
   friendsList.innerHTML = `<p style="color:var(--muted);font-size:0.85rem;">${trMsg('Még nincs elfogadott barátod. Hívd meg őket a fenti gombbal!')}</p>`;
  } else {
